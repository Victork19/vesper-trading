...
class MetricsEngine:
 ...
 def outcome(self,d, pnl, clv, quality=1.0, resolved_yes=None, attribution=None):
  ...
  for name,value in categories.items():
   ...
   existing.attribution_error[name]=((existing.attribution_error.get(name,0.0)*prior)+float(value))/existing.decisions
  attribution_metrics=(attribution or {}).get('metrics',{})
  regret=max(0.0,float(attribution_metrics.get('best_available_alternative_utility',0) or 0)-float(attribution_metrics.get('selected_expected_utility',0) or 0))
  execution_error=float(categories.get('execution_error',0) or 0)
  existing.counterfactual_regret_sum+=regret;existing.execution_error_sum+=execution_error
